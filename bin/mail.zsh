...
mmail() {
  ...
  search_query='
    ...
    if [[ "{q}" = "'\'''\''" || "{q}" = "$(echo {)q}" ]]; then
      script -q -f -e --command "himalaya --account "${current_account}" list -s 100 --max-width 500| tail -n +2" /dev/null
    else
      ...
      script -q -f -e --command himalaya --account "${current_account}" list -s 100 --max-width "'$(tput cols)'" $query /dev/null | tail -n +2
    fi
  '
  ...
  SHELL=/bin/sh FZF_DEFAULT_COMMAND="$search_query" fzf \
    --disabled \
